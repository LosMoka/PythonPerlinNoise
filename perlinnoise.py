...
# Calcul un bruit à la bonne résulution à partir du bruit de base en faisant un sous-échantillonnage du bruit.
# C'est-à-dire, calcul la version "zommée" du bruit de base
def makeSmoothNoise(baseNoise,octave):
    ...
    samplePeriod = pow(2, octave)                                   # 2)Période d'échantillonnage : correspond à de combien l'image sera zommé, on prend un point tous les samplePeriod
    ...
    for i in range(w):                                              # 3)On définie un rectangle (sample_i0,sample_j0),(sample_i1,sample_j0),(sample_i1,sample_j1),(sample_i0,sample_j1)
                                                                    ...
                                                                    # calculer sa valeur en fonction de sa distance par rapport aux quatre sommets du rectangle
        ...
        for j in range(h):
            ...
            # On pondère par la distance horizontale du point qu'on est en train de traiter, (i,j) par rapport à (sample_i0,sample_j0) et (sample_i1,sample_j0)
...
def makePerlinNoise(w,h, octave, amplitude, persistance):
    baseNoise = makeNoise(w,h)                                      # Génère un bruit aléatoire de taille w,h
    smoothNoises = []
    amplitudeTot = 0
    ...
    for k in reversed(range(octave)):                               # Fusion des versions zoomées
        ...
        for i in range(w):
            for j in range(h):
                perlinNoise[i][j] += amplitude*smoothNoises[k][i][j] # On attenue l'effet de chaque zoom en multipliant ces valeurs par l'amplitude
